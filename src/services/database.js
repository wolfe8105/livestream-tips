...
class Database {
  ...
  // Profile icon
  getProfileIcon() { return this._get('sts_profile_icon') || '😎'; }
  setProfileIcon(icon) { this._set('sts_profile_icon', icon); }
  getPurchasedIcons() {
    const val = this._get('sts_purchased_icons');
    return val ? JSON.parse(val) : [];
  }
  setPurchasedIcons(icons) { this._set('sts_purchased_icons', JSON.stringify(icons)); }
  getTotalPurchased() {
    const txns = this.getTransactions(9999);
    return txns.filter(t => t.type === 'purchase' && t.amount > 0).reduce((s, t) => s + t.amount, 0) || 1200;
  }
  ...
  // ============================================
  // ACCESS TIERS — Freemium Gating
  // ============================================
  // Guest: no account — browse + watch 480p only
  // Free:  signed up, 0 tokens — chat, follow 3 performers
  // Paid:  has tokens — HD, private shows, DMs, unlimited follows
  ...
  getAccessTier(user) {
    ...
    // 'paid' if they have tokens OR have ever purchased (check tx history)
    if (bal > 0) return 'paid';
    const txns = this.getTransactions(10);
    const hasPurchased = txns.some(t => t.type === 'purchase' || t.type === 'token-purchase');
    if (hasPurchased) return 'paid';
    return 'free';
  }

  canFollow(user) {
    const tier = this.getAccessTier(user);
    if (tier === 'guest') return { allowed: false, reason: 'signup', message: 'Create a free account to follow performers' };
    if (tier === 'paid') return { allowed: true };
    // Free tier — check limit
    const count = this.getFollowing().length + this.getFavorites().length;
    if (count >= this.FREE_FOLLOW_LIMIT) {
      return { allowed: false, reason: 'tokens', message: `Free accounts can follow up to ${this.FREE_FOLLOW_LIMIT} performers. Buy tokens to unlock unlimited follows.` };
    }
    return { allowed: true };
  }

  canChat(user) {
    if (!user) return { allowed: false, reason: 'signup', message: 'Create a free account to chat' };
    return { allowed: true };
  }

  canPrivateShow(user) {
    if (!user) return { allowed: false, reason: 'signup', message: 'Create an account and buy tokens for private shows' };
    return { allowed: true }; // Token check happens at purchase time
  }

  canPrivateMessage(user) {
    if (!user) return { allowed: false, reason: 'signup', message: 'Create an account to send messages' };
    const tier = this.getAccessTier(user);
    if (tier === 'free') return { allowed: false, reason: 'tokens', message: 'Buy tokens to unlock private messages' };
    return { allowed: true };
  }

  canHD(user) {
    if (!user) return { allowed: false, reason: 'signup', label: '480p', message: 'Sign up and buy tokens for HD streaming' };
    const tier = this.getAccessTier(user);
    if (tier === 'free') return { allowed: false, reason: 'tokens', label: '480p', message: 'Buy tokens to unlock 720p HD streaming' };
    return { allowed: true, label: '720p HD' };
  }
  ...
  // ============================================
  getAgeGateStatus() {
    const stored = this._get('sts_age_verified');
    if (!stored) return null;
    try { return JSON.parse(stored); } catch { return null; }
  }

  getCookieConsent() {
    const stored = this._get('sts_cookie_consent');
    if (!stored) return null;
    try { return JSON.parse(stored); } catch { return null; }
  }
