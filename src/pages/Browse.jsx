...
export default function Browse() {
  ...
  const sortedStates = Object.keys(STATE_NAMES)
    .filter(abbr => {
      if (liveOnly && (!counts[abbr] || counts[abbr].live === 0)) return false;
      if (stateFilter) {
        const name = STATE_NAMES[abbr].toLowerCase();
        const q = stateFilter.toLowerCase();
        if (!name.includes(q) && !abbr.toLowerCase().includes(q)) return false;
      }
      return true;
    })
    .sort((a, b) => {
      if (sortBy === 'alpha') return STATE_NAMES[a].localeCompare(STATE_NAMES[b]);
      if (sortBy === 'clubs') return (CLUBS[b]?.length || 0) - (CLUBS[a]?.length || 0);
      // default: viewers (live first)
  ...
  return (
    <>
      ...
      <div className="hero">
        ...
        <p>Every performer works at a real venue you can walk into. No AI. No filters. No fakes. See them live on stage.</p>
      </div>
      ...
      <div className="trust-banner">
        <div style={{
          ...
          <button onClick={() => setLiveOnly(!liveOnly)} style={{
            padding: '5px 12px', borderRadius: 8, fontSize: 11, fontWeight: 700,
            background: liveOnly ? 'var(--red)' : 'rgba(255,255,255,0.06)',
            color: liveOnly ? '#fff' : 'var(--muted)',
          ...
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} style={{
            ...
            <option value="clubs">Sort: Most Clubs</option>
            <option value="alpha">Sort: A-Z</option>
          </select>

          <button onClick={() => setShowFilters(!showFilters)} style={{
            padding: '5px 12px', borderRadius: 8, fontSize: 11, fontWeight: 700,
            background: stateFilter ? 'var(--violet)' : 'rgba(255,255,255,0.06)',
            color: stateFilter ? '#fff' : 'var(--muted)',
            border: 'none', cursor: 'pointer',
          }}>
            🗺️ Filter{stateFilter ? ': ' + stateFilter : ''}
          </button>

          {(liveOnly || stateFilter) && (
            <button onClick={() => { setLiveOnly(false); setStateFilter(''); setSortBy('viewers'); }} style={{
              padding: '5px 10px', borderRadius: 8, fontSize: 11,
              background: 'rgba(225,29,72,0.1)', border: 'none',
      ...
      {showFilters && (
        <div style={{ padding: '0 16px', marginTop: -8, marginBottom: 12 }}>
          <input
            type="text"
            placeholder="Type state name to filter..."
            value={stateFilter}
          ...
          {stateFilter && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
              {Object.entries(STATE_NAMES)
                .filter(([a, n]) => n.toLowerCase().includes(stateFilter.toLowerCase()) || a.toLowerCase().includes(stateFilter.toLowerCase()))
                .slice(0, 8)
                .map(([abbr, name]) => (
                  <button key={abbr} onClick={() => { setStateFilter(name); setShowFilters(false); }} style={{
      ...
      <div className="stats-bar">
        ...
        <div className="stat clubs" onClick={() => navigate('/clubs')} style={{ cursor: 'pointer', color: 'var(--violet)', textDecoration: 'underline' }}>667 Clubs</div>
      </div>

      {/* State Grid */}
      <div className="section-label">
        {stateFilter || liveOnly ? `${sortedStates.length} States` : 'All States'}
        {liveOnly && ' (Live Only)'}
      </div>
