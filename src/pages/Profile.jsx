...
export default function Profile() {
  ...
  function toggleFavorite() {
    if (isFav) {
      ...
      if (!check.allowed) { setGatePrompt({ reason: check.reason, message: check.message }); return; }
      db.addFavorite({
  ...
  return (
    <>
      ...
      <div className="page-pad">
        ...
        <button className="btn-primary mb-12" onClick={() => {
          const check = db.canPrivateShow(user);
          if (!check.allowed) { setGatePrompt({ reason: check.reason, message: check.message }); return; }
          alert(`📅 Book Private Show\n\n${p.name}\n\nComing soon!`);
        }}>📅 Book Private Show</button>
        <button className="btn-pink-lg" onClick={() => {
          const check = db.canPrivateMessage(user);
          if (!check.allowed) { setGatePrompt({ reason: check.reason, message: check.message }); return; }
          alert(`💌 Send Message\n\n${p.name}\n\nComing soon!`);
        }}>💌 Send Message</button>
      </div>
